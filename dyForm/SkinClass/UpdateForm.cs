namespace dyForm.SkinClass
{
    ...
    public class UpdateForm
    {
        ...
        {
            int hRgn = dyForm.Win32.NativeMethods.CreateRoundRectRgn(0, 0, ctrl.ClientRectangle.Width + 1, ctrl.ClientRectangle.Height + 1, RgnRadius, RgnRadius);
            dyForm.Win32.NativeMethods.SetWindowRgn(ctrl.Handle, hRgn, true);
        }
            ...
            if (roundStyle != RoundStyle.None)
            {
                using (GraphicsPath path = GraphicsPathHelper.CreatePath(rect, radius, roundStyle, true))
                {
                    using (Region region = new Region(path))
                    {
                        ...
                        region.Union(path);
                        IntPtr windowDC = dyForm.Win32.NativeMethods.GetWindowDC(hWnd);
                        try
                        {
                            using (Graphics graphics = Graphics.FromHdc(windowDC))
                            {
                                dyForm.Win32.NativeMethods.SetWindowRgn(hWnd, region.GetHrgn(graphics), redraw);
                            }
                        }
                        finally
                        {
                            dyForm.Win32.NativeMethods.ReleaseDC(hWnd, windowDC);
                        }
                    }
                    return;
                }
            }
            IntPtr hRgn = dyForm.Win32.NativeMethods.CreateRectRgn(0, 0, rect.Width, rect.Height);
            dyForm.Win32.NativeMethods.SetWindowRgn(hWnd, hRgn, redraw);
        }
        ...
        public static void CursorClick(int x, int y)
        {
            int dwFlags = 2;
            int num2 = 4;
            dyForm.Win32.NativeMethods.mouse_event(dwFlags, (x * 0x10000) / 0x400, (y * 0x10000) / 0x300, 0, 0);
            dyForm.Win32.NativeMethods.mouse_event(num2, (x * 0x10000) / 0x400, (y * 0x10000) / 0x300, 0, 0);
        }
                    ...
                    using (Graphics graphics2 = Graphics.FromImage(bitmap2))
                    {
                        using (SolidBrush brush = new SolidBrush(Color.FromArgb(0x10, ColorBack.R, ColorBack.G, ColorBack.B)))
                        {
                            using (SolidBrush brush2 = new SolidBrush(ColorFore))
                            {
                                ...
                                bitmap = new Bitmap(bitmap2.Width + BlurConsideration, bitmap2.Height + BlurConsideration);
