namespace dyForm.CControl
{
    ...
    public class RtfRichTextBox : RichTextBox
    {
        ...
        {
            StringBuilder builder = new StringBuilder();
            int num = (int) Math.Round((double) ((((float) _image.Width) / this.xDpi) * 2540f));
            int num2 = (int) Math.Round((double) ((((float) _image.Height) / this.yDpi) * 2540f));
            int num3 = (int) Math.Round((double) ((((float) _image.Width) / this.xDpi) * 1440f));
            int num4 = (int) Math.Round((double) ((((float) _image.Height) / this.yDpi) * 1440f));
        ...
        public void InsertEmotion()
        {
            if (hasGdiPlus)
            {
                ...
                {
                    int start = base.Find(str, RichTextBoxFinds.None);
                    if (start > -1)
                    {
                        base.Select(start, str.Length);
                        this.InsertImage(this.emotions[str]);
                    }
                }
            }
        }
