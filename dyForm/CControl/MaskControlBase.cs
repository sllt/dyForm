namespace dyForm.CControl
{
    ...
    internal abstract class MaskControlBase : NativeWindow, IDisposable
    {
        ...
        internal void CheckBounds(IntPtr hWnd)
        {
            ...
            dyForm.Win32.Struct.RECT rect2 = new dyForm.Win32.Struct.RECT();
            dyForm.Win32.NativeMethods.GetWindowRect(base.Handle, ref rect2);
            dyForm.Win32.NativeMethods.GetWindowRect(hWnd, ref lpRect);
            uint flags = 0x214;
            if (!dyForm.Win32.NativeMethods.EqualRect(ref lpRect, ref rect2))
            {
                dyForm.Win32.NativeMethods.Point lpPoint = new dyForm.Win32.NativeMethods.Point(lpRect.Left, lpRect.Top);
                dyForm.Win32.NativeMethods.ScreenToClient(dyForm.Win32.NativeMethods.GetParent(base.Handle), ref lpPoint);
                dyForm.Win32.NativeMethods.SetWindowPos(base.Handle, IntPtr.Zero, lpPoint.x, lpPoint.y, lpRect.Right - lpRect.Left, lpRect.Bottom - lpRect.Top, flags);
            }
        }

        internal void CreateParamsInternal(IntPtr hWnd)
        {
            IntPtr parent = dyForm.Win32.NativeMethods.GetParent(hWnd);
            dyForm.Win32.Struct.RECT lpRect = new dyForm.Win32.Struct.RECT();
            dyForm.Win32.NativeMethods.Point lpPoint = new dyForm.Win32.NativeMethods.Point();
            dyForm.Win32.NativeMethods.GetWindowRect(hWnd, ref lpRect);
            lpPoint.x = lpRect.Left;
            lpPoint.y = lpRect.Top;
            dyForm.Win32.NativeMethods.ScreenToClient(parent, ref lpPoint);
            int num = 0x5400000d;
            int num2 = 0x88;
        ...
        internal void SetVisibale(bool visibale)
        {
            if (visibale)
            {
                dyForm.Win32.NativeMethods.ShowWindow(base.Handle, 1);
            }
            else
            {
                dyForm.Win32.NativeMethods.ShowWindow(base.Handle, 0);
            }
        }

        internal void SetVisibale(IntPtr hWnd)
        {
            bool visibale = (dyForm.Win32.NativeMethods.GetWindowLong(hWnd, -16) & 0x10000000) == 0x10000000;
        ...
        {
            uint flags = 0x213;
            dyForm.Win32.NativeMethods.SetWindowPos(base.Handle, HWND.HWND_TOP, 0, 0, 0, 0, flags);
        }
